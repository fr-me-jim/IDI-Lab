...
MyGLWidget::MyGLWidget (QWidget* parent) : QOpenGLWidget(parent), program(NULL)
{
  ...
  colorLlum = glm::vec3(0.8, 0.8, 0.8);
  posFocus = glm::vec3(1.0, 0.0, 1.0);
  posFocus1 = glm::vec3(-1.5, 3.0, 1.5);
  posFocus2 = glm::vec3(1.5, 3.0, 1.5);
  despl = 0.0f;
  tipusFocus = 0;
}
...
void MyGLWidget::initializeGL ()
{
  ...
  // si els uniforms es calculen dinamicament, pasarlos al model transform
  glUniform3fv(colorLoc,1,&colorLlum[0]); //color llum uniform 
}
...
void MyGLWidget::carregaShaders()
{
  ...
  tipusLoc = glGetUniformLocation (program->programId(), "tipus");
  ...
  focusLoc = glGetUniformLocation(program->programId(), "focus");
  normMatLoc = glGetUniformLocation(program->programId(), "nMat");
}

void MyGLWidget::calculaFocus() {
    if(tipusFocus == 0) {
        //focus de camera / escena /model
        posFocusSCO =   glm::vec4(posFocus,1.0); //view* es d'escena / view*tg*posFocus de model
        glUniform4fv(focusLoc,1,&posFocusSCO[0]); //uniform de posFocus
  }
  if (tipusFocus == 1)  {
          //focus de camera / escena /model
          posFocusSCO = View * glm::vec4(posFocus1,1.0); //view* es d'escena / view*tg*posFocus de model
          glUniform4fv(focusLoc,1,&posFocusSCO[0]); //uniform de posFocus
  }
  if (tipusFocus == 2)  {
          //focus de camera / escena /model
          posFocusSCO = View * glm::vec4(posFocus2,1.0); //view* es d'escena / view*tg*posFocus de model
          glUniform4fv(focusLoc,1,&posFocusSCO[0]); //uniform de posFocus
  }
}
...
void MyGLWidget::modelTransformModel ()
{
  ...
  TG = glm::translate(TG, glm::vec3(-1.5-despl, -2.0, 1.5+despl));
  TG = glm::rotate(TG,float(M_PI-M_PI/4.0),glm::vec3(0,1,0));
  TG = glm::scale(TG, glm::vec3(escala, escala, escala));
  TG = glm::translate(TG, -centreBasePatr);

  
  //Uniforms il·luminació
  nMatrix = glm::inverseTranspose(glm::mat3(View * TG));
  glUniformMatrix3fv(normMatLoc,1,GL_FALSE,&nMatrix[0][0]); //uniform de la nMatrix
  
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &TG[0][0]);
}
void MyGLWidget::modelTransformModelPetit ()
{
  TG = glm::mat4(1.f);  // Matriu de transformació
  TG = glm::translate(TG, glm::vec3(1.5+despl, -2.0, 1.5+despl));
  TG = glm::rotate(TG,float(M_PI+M_PI/4.0),glm::vec3(0,1,0));
  ...
  //Uniforms il·luminació, un cop s'entra al modelTransform, la View i la proj estan actualitzades
  nMatrix = glm::inverseTranspose(glm::mat3(View * TG));
  glUniformMatrix3fv(normMatLoc,1,GL_FALSE,&nMatrix[0][0]); //uniform de la nMatrix
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  case Qt::Key_F: { // canvia òptica entre perspectiva i axonomètrica
   ...
   case Qt::Key_Up: { // canvia òptica entre perspectiva i axonomètrica
     //if((despl + 0.25) >= 1.5 || (despl + 0.25) <= 3 )
               despl -= 0.25;
     /*if((despl - 0.25 ) >= -1.5 || (despl + 0.25) <= -3 )
               despl += 0.25;*/
     break;
  ...
  case Qt::Key_Down: { // canvia òptica entre perspectiva i axonomètrica
   //if((despl + 0.25) <= -1.5 && (despl + 0.25) >= 1.5 )
      despl += 0.25;
    break;
  }
